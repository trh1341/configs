...
myLogHook h = defaultPP
    { ppCurrent	    = dzenColor color15 color1 . pad . ("^i(.dzen/corner.xbm)" ++) -- current workspace
    , ppVisible	    = dzenColor color11 color2                 . pad                               -- visible workspaces on other screens
    , ppHidden	    = dzenColor color14 ""                      . pad . ("^i(.dzen/corner.xbm)" ++) -- hidden workspaces with apps
    , ppHiddenNoWindows     = dzenColor color2  ""           . pad                               -- empty workspaces
    , ppUrgent	    = dzenColor "" myUrgentBGColor                                                  -- urgent workspaces
    , ppTitle       = dzenColor color2 ""              . pad . dzenEscape                  -- title of selected window
    , ppWsSep       = ""                                                                            -- workspace seperator
    , ppSep         = dzenEscape "|"                                                                -- workspace/layout/title seperator

    -- Layout icons
    , ppLayout      = dzenColor color2 "" .
        (\ x -> case x of
...
myDzenFGColor         = color3
myNormalFGColor       = color1
myNormalBGColor       = color0
myUrgentBGColor       = "#61C24F"
